...
from ytflac.core.provider_stats import (
    ProviderScorer,
    record_success,
    record_failure,
    prioritize,
...
def test_history_manager():
    ...
    downloaded = history.get_downloaded_ids()
    failed = history.get_failed_ids()
    print(f"Downloaded IDs: {downloaded}")
    print(f"Failed IDs: {failed}")

    assert "spotify1" in downloaded
    assert "spotify2" not in downloaded
    assert "spotify2" in failed
    assert "spotify1" not in failed

    # Re-add same track as success — should overwrite failure
    history.add_download(
        track_id="spotify2",
        track_name="Song 2",
        artist_name="Artist 2",
        provider="qobuz",
        success=True,
        file_path="/tmp/song2.flac",
    )
    downloaded2 = history.get_downloaded_ids()
    assert "spotify2" in downloaded2
