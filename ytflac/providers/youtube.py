...
class YouTubeProvider(BaseProvider):
    ...
    def _get_youtube_url(
        self, spotify_track_id: str, track_name: str = "", artist_name: str = ""
    ) -> str:
        ...
        raise RuntimeError(
            "Failed to resolve YouTube URL via Songlink and direct search"
        )
    ...
    def _embed_metadata(
        ...
        try:
            ...
            if genre:
                audio.add(TCON(encoding=3, text=str(genre)))
            if bpm:
                audio.add(TBPM(encoding=3, text=str(bpm)))

            audio.add(
                TRCK(
                    encoding=3, text=f"{_safe_int(track_num)}/{_safe_int(total_tracks)}"
                )
            )
            audio.add(
                TPOS(encoding=3, text=f"{_safe_int(disc_num)}/{_safe_int(total_discs)}")
            )

            if publisher:
                audio.add(TPUB(encoding=3, text=[str(publisher)]))
            if url:
                audio.add(WXXX(encoding=3, desc="", url=str(url)))
    ...
    def download_track(
        ...
        **kwargs,
        ...
        try:
            ...
            dl_url = self._request_spotube_dl(video_id) or self._request_cobalt(
                video_id
            )
            if not dl_url:
                return DownloadResult.fail(
                    self.name, "All YouTube download APIs failed"
                )
            ...
            artist = (
                metadata.artists.split(",")[0].strip()
                if first_artist_only
                else metadata.artists
            )
            album_artist = (
                metadata.album_artist.split(",")[0].strip()
                if first_artist_only
