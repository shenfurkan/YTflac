...
_CLIENT_SECRET = base64.b64decode(
    "OWJiOWUxMzFmZjI4NDI0Y2I2YTQyMGFmZGY0MWQ0NGE="
).decode()
...
class SpotifyMetadataClient:
    ...
    def _get(self, path: str, **kwargs) -> dict:
        ...
        url = f"{_API_BASE}/{path.lstrip('/')}" if not path.startswith("http") else path
        ...
        for attempt in range(3):
            ...
            if resp.status_code == 429:
                ...
                logger.info(
                    "[spotify] Rate limited — sleeping %ss (attempt %d/3)",
                    retry_after,
                    attempt + 1,
                )
        ...
        raise NetworkError("spotify", f"Rate-limited after 3 retries on {path}")
    ...
    def get_album_tracks(self, album_id: str) -> tuple[dict, list[TrackMetadata]]:
        ...
        missing_ids = [
            it["id"] for it in items if it.get("id") and not get_cached_isrc(it["id"])
        ]
    ...
    def get_playlist_tracks(self, playlist_id: str) -> tuple[dict, list[TrackMetadata]]:
        ...
        for item in self._paginate(
            f"{_API_BASE}/playlists/{playlist_id}/tracks?limit=100"
        ):
        ...
        # Batch-resolve ISRCs for playlist tracks exactly as we do for albums.
        # Playlist API items don't include ISRC in external_ids, so we must
        # fetch individual track details concurrently.
        missing_ids = [t.id for t in tracks if not t.isrc]
        ...
        if missing_ids:
            ...
            with ThreadPoolExecutor(max_workers=min(8, len(missing_ids))) as pool:
                for tid, isrc in pool.map(_fetch_isrc, missing_ids):
                    if isrc:
                        isrc_map[tid] = isrc

            # Patch missing ISRCs back onto the TrackMetadata objects
            if isrc_map:
                tracks = [
                    t.model_copy(update={"isrc": isrc_map[t.id]})
                    if (t.id in isrc_map and not t.isrc)
                    else t
                    for t in tracks
                ]
    ...
    def _format_artists(artists: list[dict] | str) -> str:
        ...
        return ", ".join(
            a.get("name", "Unknown") if isinstance(a, dict) else str(a) for a in artists
        )
    ...
    def _track_from_raw(self, data: dict) -> TrackMetadata:
        ...
        album_artists = self._format_artists(
            album.get("artists", []) or data.get("artists", [])
        )
