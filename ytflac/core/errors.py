...
_KIND_LABELS: dict[ErrorKind, tuple[str, str]] = {
    ...
    ErrorKind.NETWORK_ERROR    : ("Network error",
                                  "A request to the provider failed. Check your internet "
                                  "connection / VPN and try again."),
    ErrorKind.PARSE_ERROR      : ("Provider response changed",
                                  "The provider's API format changed and we could not parse it. "
                                  "An app update may be required."),
    ErrorKind.UNAVAILABLE      : ("Track unavailable",
                                  "The track exists but cannot be downloaded right now (region lock, "
                                  "removed, or DRM-only)."),
    ErrorKind.FILE_IO          : ("File system error",
                                  "Could not write the file. Check the output folder permissions or free disk space."),
    ErrorKind.INVALID_URL      : ("Invalid URL",
                                  "The URL format is not supported."),
    ErrorKind.METADATA_ERROR   : ("Metadata error",
                                  "Could not read or write track tags."),
}

_RAW_PATTERNS: list[tuple[ErrorKind, _re.Pattern[str]]] = [
    (ErrorKind.AUTH_FAILED,    _re.compile(r"\b(401|403|forbidden|unauthor|auth.?fail|invalid.?token)\b", _re.I)),
    (ErrorKind.RATE_LIMITED,   _re.compile(r"\b(429|rate.?limit|too.?many|retry.?after)\b", _re.I)),
    (ErrorKind.TRACK_NOT_FOUND,_re.compile(r"\b(404|not.?found|no.?match|isrc.?missing)\b", _re.I)),
    (ErrorKind.NETWORK_ERROR,  _re.compile(r"\b(timeout|timed.?out|connection|connect.?reset|"
                                            r"connection.?aborted|getaddrinfo|name.?resolution|ssl|"
                                            r"5\d\d|bad.?gateway|service.?unavailable|gateway.?timeout)\b", _re.I)),
    (ErrorKind.UNAVAILABLE,    _re.compile(r"\b(unavailable|geo.?block|region|drm|premium.?only)\b", _re.I)),
    (ErrorKind.FILE_IO,        _re.compile(r"\b(no.?space|permission.?denied|disk.?full|file.?exists|"
                                            r"errno|oserror)\b", _re.I)),
    (ErrorKind.PARSE_ERROR,    _re.compile(r"\b(json|parse|decode|unexpected.?response|malformed)\b", _re.I)),
    (ErrorKind.METADATA_ERROR, _re.compile(r"\b(mutagen|tag|id3|metadata|cover.?art)\b", _re.I)),
]
...
def friendly_explanation(kind: ErrorKind) -> str:
    return _KIND_LABELS.get(kind, ("Unknown error",
                                    "An unrecognised error occurred. See the debug log."))[1]
