...
class _LyricsCache:
    ...
    def _get_cache_key(
        self, track_id: str, isrc: str, track_name: str, artist_name: str
    ) -> str:
                    ...
                    for key, value in data.items():
                        ...
                        if expires_at > now:
                            self._cache[key] = _CacheEntry(
                                value["lyrics"], value["provider"], expires_at
                            )
                logger.debug(
                    "[lyrics/cache] loaded %d entries from disk", len(self._cache)
                )
...
def _apple_search_song_id(
    track_name: str, artist_name: str, duration_s: int, timeout: int
) -> str | None:
    """Arama yapıp en iyi eşleşen şarkının ID'sini döndürür."""
...
def _apple_parse_lrc(content: list) -> str:
    """Apple API'sinden gelen karmaşık söz verisini standart LRC formatına dönüştürür."""
    lrc_lines = []
    for line in content:
...
def _fetch_apple(
    track_name: str, artist_name: str, duration_s: int, timeout: int = 15
) -> str:
    """Apple Music sözlerini çeker. İşlemi arama ve ayrıştırma olarak ikiye böler."""
    try:
        song_id = _apple_search_song_id(track_name, artist_name, duration_s, timeout)
        if not song_id:
            return ""
        ...
        return _apple_parse_lrc(content)
...
def _fetch_musixmatch(
    ...
    params = {"t": track_name, "a": artist_name, "type": "word", "format": "lrc"}
    if duration_s > 0:
...
def fetch_lyrics(
    ...
    for provider in providers:
        ...
        if result and result.strip():
            ...
            # Cache the result
            _lyrics_cache.set(
                track_id, isrc, track_name, artist_name, result.strip(), provider
            )
