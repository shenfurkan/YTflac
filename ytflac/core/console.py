...
def print_track_header(position: int, total: int, title: str, artists: str, album: str,
...
def print_source_banner(provider: str, api: str, quality: str,
                         log_callback: Callable[[str, str], None] | None = None) -> None:
...
def print_official_source(provider: str, quality: str,
                           log_callback: Callable[[str, str], None] | None = None) -> None:
...
def print_summary(
    ...
    print(f"╚{bar}╝")
    if log_callback:
        log_callback(f"Finished — {succeeded} ok, {len(failed)} failed in {_fmt_seconds(elapsed_s)}", "info")


def print_skip(filepath: str, size_mb: float,
               log_callback: Callable[[str, str], None] | None = None) -> None:
...
def print_api_failure(provider: str, api: str, reason: str,
                     log_callback: Callable[[str, str], None] | None = None) -> None:
    label = _shorten_api(api)
...
def print_quality_fallback(provider: str, from_q: str, to_q: str,
                            log_callback: Callable[[str, str], None] | None = None) -> None:
    print(f"  ⬇  {provider}: quality {from_q} not available — fallback → {to_q}")
    if log_callback:
        log_callback(f"{provider}: {from_q} not available, falling back to {to_q}", "warning")
